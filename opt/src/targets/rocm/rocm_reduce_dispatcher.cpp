...
std::string op_tag(ReductionOperation op) {
    ...
    throw InvalidSDFGException("ROCMReduceDispatcher: unknown reduction operation");
}
...
std::string identity_literal(ReductionOperation op, types::PrimitiveType prim) {
    ...
    if (floating) {
        // INFINITY / -INFINITY are provided by HIP's math headers.
    ...
    throw InvalidSDFGException("ROCMReduceDispatcher: unsupported integer width for min/max reduction");
}
    ...
    if (auto* ptr = dynamic_cast<const types::Pointer*>(&type)) {
        if (!ptr->has_pointee_type()) {
            throw InvalidSDFGException(
                "ROCMReduceDispatcher: reduction accumulator '" + container + "' has no pointee type"
            );
        }
        ...
        throw InvalidSDFGException(
            "ROCMReduceDispatcher: reduction accumulator '" + container + "' must point to a scalar"
        );
    }
    throw InvalidSDFGException(
        "ROCMReduceDispatcher: reduction accumulator '" + container +
...
symbolic::Expression accumulator_index(
        ...
        if (auto* block = dynamic_cast<structured_control_flow::Block*>(current)) {
            ...
            for (auto& memlet : dfg.edges()) {
                ...
                if (!found) {
                    ...
                    throw InvalidSDFGException(
                        "ROCMReduceDispatcher: accumulator '" + container +
    ...
    if (!found) {
        throw InvalidSDFGException(
            "ROCMReduceDispatcher: accumulator '" + container + "' is not accessed in the reduce body"
        );
    }
    if (symbolic::uses(index, indvar)) {
        throw InvalidSDFGException(
            "ROCMReduceDispatcher: accumulator '" + container + "' index depends on the reduction variable '" +
...
// Per-dimension HIP built-in symbols.
struct DimSymbols {
...
DimSymbols dim_symbols(ROCMDimension dim) {
    switch (dim) {
    ...
    throw InvalidSDFGException("ROCMReduceDispatcher: invalid ROCM dimension");
}

// Whether the HIP runtime provides a native atomicAdd overload for this primitive.
bool has_native_atomic_add(types::PrimitiveType prim) {
    const size_t width = types::bit_width(prim);
    if (types::is_floating_point(prim)) {
        return width == 32 || width == 64; // float, double
    }
    if (width == 32) {
...
bool ROCMReduceDispatcher::is_nested_in_rocm_kernel() {
    ...
    while (ancestor != nullptr) {
        if (auto* map = dynamic_cast<structured_control_flow::Map*>(ancestor)) {
            if (map->schedule_type().value() == ScheduleType_ROCM::value()) {
                return true;
            }
        }
...
void ROCMReduceDispatcher::dispatch_node(
    codegen::PrettyPrinter& main_stream,
    codegen::PrettyPrinter& globals_stream,
    codegen::CodeSnippetFactory& library_snippet_factory
) {
    if (node_.reductions().empty()) {
        throw InvalidSDFGException("ROCMReduceDispatcher: Reduce node carries no reductions");
    }

    // Nested inside an enclosing ROCm kernel: inline the privatize + grid-stride
    // reduce + atomic merge on this Reduce's own GPU dimension. No kernel launch.
    if (this->is_nested_in_rocm_kernel()) {
        ROCMDimension reduce_dim = ScheduleType_ROCM::dimension(node_.schedule_type());
    ...
    symbolic::Integer block_size = ScheduleType_ROCM::block_size(node_.schedule_type());
    ...
    this->dispatch_reduce_core(
        library_snippet_factory, library_stream, ROCMDimension::X, /*declare_scope_variables=*/true, scope_variables
    );
...
void ROCMReduceDispatcher::dispatch_header(
...
void ROCMReduceDispatcher::dispatch_kernel_call(
    codegen::PrettyPrinter& main_stream,
...
void ROCMReduceDispatcher::dispatch_reduce_core(
    codegen::CodeSnippetFactory& library_snippet_factory,
    codegen::PrettyPrinter& library_stream,
    ROCMDimension dimension,
    ...
    library_stream << "int " << start_var << " = " << rocm_language_extension.expression(flat_id) << ";" << std::endl;
    library_stream << "int " << step_var << " = " << rocm_language_extension.expression(num_threads) << ";"
                   << std::endl;
    ...
    for (auto& reduction : node_.reductions()) {
        ...
        std::string ctype = rocm_language_extension.primitive_type(prim);
    ...
    for (auto& reduction : node_.reductions()) {
        ...
        if (symbolic::eq(index, symbolic::zero())) {
            ...
            library_stream << ctype << " *" << reduction.container << " = &__daisy_reduce_" << reduction.container
                           << " - (" << rocm_language_extension.expression(index) << ");" << std::endl;
        }
    }
    ...
    library_stream << "for (int " << indvar->get_name() << " = " << rocm_language_extension.expression(loop_init)
                   << "; " << rocm_language_extension.expression(node_.condition()) << "; " << indvar->get_name()
                   << " = " << rocm_language_extension.expression(loop_update) << ") {" << std::endl;
    library_stream.setIndent(library_stream.indent() + 4);
...
void ROCMReduceDispatcher::dispatch_atomic_merge(
    codegen::PrettyPrinter& library_stream,
    codegen::CodeSnippetFactory& library_snippet_factory,
    ...
    auto index = accumulator_index(node_.root(), reduction.container, node_.indvar());
    std::string index_str = rocm_language_extension.expression(index);
    ...
    if (width != 32 && width != 64) {
        throw InvalidSDFGException(
            "ROCMReduceDispatcher: only 32/64-bit reduction accumulators are supported in the atomics baseline"
        );
    }
...
codegen::InstrumentationInfo ROCMReduceDispatcher::instrumentation_info() const {
    auto& loop_analysis = analysis_manager_.get<analysis::LoopAnalysis>();
