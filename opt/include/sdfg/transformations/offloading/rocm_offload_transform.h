...
class ROCMOffloadTransform : public gpu::GPUOffloadTransform<ROCMDataOffloadingNode> {
public:
    explicit ROCMOffloadTransform(
        structured_control_flow::StructuredLoop& loop,
        symbolic::Integer parallel_size,
        gpu::TargetLevel target_level = gpu::TargetLevel::X_GRID,
        bool allow_dynamic_sizes = false
    )
        : gpu::GPUOffloadTransform<ROCMDataOffloadingNode>(loop, parallel_size, target_level, allow_dynamic_sizes) {};

    std::string name() const override;

    static ROCMOffloadTransform from_json(builder::StructuredSDFGBuilder& builder, const nlohmann::json& desc);
