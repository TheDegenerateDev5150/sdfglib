 ...
 * @ref ScheduleType_CUDA, i.e. the reduction dimension is parallelized across
 ...
 */
class CUDAReduceDispatcher : public codegen::NodeDispatcher {
private:
    structured_control_flow::Reduce& node_;

    /**
     * @brief True if this Reduce is nested inside an enclosing CUDA kernel
     *        (an ancestor Map with @ref ScheduleType_CUDA).
     */
    bool is_nested_in_cuda_kernel();
