...
def setup_begin(batch_size: int = 32):
    model = Resnet18Begin()
    model.eval()
...
def setup_end(batch_size: int = 32):
    model = Resnet18End()
    model.eval()
    x = torch.randn(batch_size, 512, 7, 7)
    return model, x


def setup_all(batch_size: int = 32):
    model = Resnet18()
    model.eval()
    x = torch.randn(batch_size, 3, 224, 224)
    return model, x
